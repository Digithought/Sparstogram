      ...
      expect(sparstogram.add(101)).to.be.greaterThan(0);
		...
		it('should compress when maxCentroids is reduced', () => {
			...
			for (const centroid of sparstogram.ascending()) {
				expect(centroid.value).to.equal(2);	// Should be exactly the mean of the original values
			}
		});
...
describe('atMarker method', () => {
  ...
  it('should return correct quantile for a compressed histogram', () => {
    ...
    const lowerQuartile = sparstogram.markerAt(0);
    expect(lowerQuartile).to.not.be.undefined;
    expect(lowerQuartile?.centroid.value).to.be.closeTo(26.5, 0.1); // Actual value is 26.5
		expect(lowerQuartile?.rank).to.equal(25); // Rank for 0.25 of 97 items (1-97) is 25

		const median = sparstogram.markerAt(1);
    expect(median).to.not.be.undefined;
    expect(median?.centroid.value).to.be.closeTo(46.5, 0.1); // Adjusted for actual compressed value
		expect(median?.rank).to.equal(49);

		const upperQuartile = sparstogram.markerAt(2);
    expect(upperQuartile).to.not.be.undefined;
    expect(upperQuartile?.centroid.value).to.be.closeTo(73, 0.1); // Adjusted for actual compressed value
  ...
  it('should interpolate in sparse histogram', () => {
    ...
    const tenthPercentile = sparstogram.markerAt(0);
    expect(tenthPercentile).to.not.be.undefined;
    expect(tenthPercentile?.value).to.be.closeTo(-51, 1);
    expect(tenthPercentile?.centroid.value).to.be.closeTo(125, 0.1);

    const ninetiethPercentile = sparstogram.markerAt(1);
    expect(ninetiethPercentile).to.not.be.undefined;
    expect(ninetiethPercentile?.value).to.be.closeTo(1051, 1);
    expect(ninetiethPercentile?.centroid.value).to.be.closeTo(875, 0.1);
  });
});
...
describe('countAt method', () => {
  ...
  it('should interpolate count in a compressed histogram', () => {
    ...
    expect(sparstogram.countAt(0)).to.equal(0);
    expect(sparstogram.countAt(1)).to.equal(1);
    expect(sparstogram.countAt(1.5)).to.equal(1);
    expect(sparstogram.countAt(2)).to.equal(1);
    expect(sparstogram.countAt(2.5)).to.equal(0);
    expect(sparstogram.countAt(3)).to.equal(0);
    expect(sparstogram.countAt(3.9)).to.equal(1);
    expect(sparstogram.countAt(4.5)).to.equal(2);
    expect(sparstogram.countAt(5.1)).to.equal(1);
    expect(sparstogram.countAt(5.5)).to.equal(0);
  });
  ...
  it('should handle very large numbers', () => {
    ...
    expect(sparstogram.countAt(1500000)).to.be.closeTo(1, 0.1);
  });
